            ...
            <table class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-blue-50/50 dark:hover:bg-gray-700/30 transition group">
                        ...
                        <td class="px-6 py-4 text-blue-900 dark:text-blue-400 font-semibold text-sm">{{ optional($event->masterEvent)->name ?? optional($event->customerCategory)->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-slate-600 dark:text-slate-400 text-sm">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-medium">
                                {{ optional(optional($event->masterEvent)->customer)->name ?? optional(optional($event->customerCategory)->customer)->name ?? '-' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-gray-600 dark:text-gray-400 text-sm font-medium">{{ optional(optional($event->masterEvent)->vehicleModel)->name ?? optional(optional(optional($event->parts->first())->product)->vehicleModel)->name ?? '-' }}</td>
